...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    hidden_valid = hidden_layer > 0
    hidden_act = hidden_valid * hidden_layer
    output_layer = np.dot(hidden_act, W2) + b2
    scores = output_layer
    pass
    #############################################################################
    #                              END OF YOUR CODE                             #
    ...
    dW2 = np.dot(hidden_act.T, prob - y_prob) / N
    db2 = np.sum(prob - y_prob, axis = 0) / N
    dh_act = np.dot(prob - y_prob, W2.T) / N

    loss += reg * np.sum(W2 ** 2) 
    dW2 += reg * W2 * 2

    dh = dh_act * hidden_valid
