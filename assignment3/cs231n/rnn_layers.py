...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # cache = ['x': x, 'Wx': Wx, 'prev_h': prev_h, 'Wh': Wh, 'b': b, 'next_h': next_h, 'forward': forward]
    cache = x, Wx, prev_h, Wh, b, forward
    pass
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    x, Wx, prev_h, Wh, b, forward = cache
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # h = np.empty((N, T, H), dtype = 'float32')
    # cache = []
    # for i in xrange(T):
    # 	if i == 0:
    # 		h[:, 0, :], cache_s = rnn_step_forward(x[:, i, :], h0, Wx, Wh, b)
    # 	else:
    # 		h[:, i, :], cache_s = rnn_step_forward(x[:, i, :], h[:, i-1, :], Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    # 	droar_h = dh[:, i, :] + dh_t
    # 	dx[:, i, :], dh_t, dWx_t, dWh_t, db_t = rnn_step_backward(droar_h, cache[i]) 
    # 	dWx += dWx_t
    # 	dWh += dWh_t
    # 	db += db_t 
    # 	dh0 = dh_t
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in xrange(T):
        if t == 0:
            h[t], prev_c, cache_t = lstm_step_forward(x[t], h0, prev_c, Wx, Wh, b)
        else:
            h[t], prev_c, cache_t = lstm_step_forward(x[t], h[t-1], prev_c, Wx, Wh, b)
