...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        h, cache_h = affine_forward(features, W_proj, b_proj)
        word, cache_w = word_embedding_forward(captions_in, W_embed)
        if self.cell_type == 'rnn':
        	rnn_h, cache_rnn = rnn_forward(word, h, Wx, Wh, b)
        	ta, cache_ta = temporal_affine_forward(rnn_h, W_vocab, b_vocab)
        elif self.cell_type == 'lstm':
        	lstm_h, cache_lstm = lstm_forward(word, h, Wx, Wh, b)
        	ta, cache_ta = temporal_affine_forward(lstm_h, W_vocab, b_vocab)


        loss, dh = temporal_softmax_loss(ta, captions_out, mask, True)


        if self.cell_type == 'rnn':
        	drnn_h, dW_vocab, db_vocab = temporal_affine_backward(dh, cache_ta)
        	dword, dh, dWx, dWh, db = rnn_backward(drnn_h, cache_rnn)
        elif self.cell_type == 'lstm':
        	dlstm_h, dW_vocab, db_vocab = temporal_affine_backward(dh, cache_ta)
        	dword, dh, dWx, dWh, db = lstm_backward(dlstm_h, cache_lstm)
        dW_embed = word_embedding_backward(dword, cache_w)
        dfeatures, dW_proj, db_proj = affine_backward(dh, cache_h)
